...
For top-K classifier metrics (P@k, recall@k), the per-day denominator MUST be `min(R(d), k)` where R(d) is the number of positives that day, NOT the count of picks actually made. The picks-made denominator silently mis-normalizes on staggered panels where R(d) < k for many days, producing artifact P@k values that have nothing to do with model skill (this is the 2026-05-28 lesson — see `[[project-r-precision-methodology]]`). The runner's `src/gbdt/topk_diagnostics.py` is fixed; post-hoc analyses must use the same denominator via `scripts/gbdt/compute_r_precision.py`.

Standard cross-cell comparison metric for gbdt experiments: **weighted R-precision** (per-day variable K = R(d); `sum(positives_caught) / sum(R(d))`). It is panel-invariant and matches actual trading-rule semantics. See `[[project-r-precision-methodology]]`.
...
- **Worktree workflow for parallel agents**: when launching long-running sub-agents that need their own working tree, the **parent agent** creates the sibling worktree (sub-agents can't `git worktree add` to sibling paths — sandbox blocks it; see `[[feedback-worktree-symlink-contract]]`). Convention: `wt-<scope>/` next to the main checkout. After `git worktree add`, the parent symlinks `data/` to the **shared scratch cache** and `.env` from the main checkout so the cache and secrets carry over. **`data/` MUST point at the scratch cache `/mnt/122CEE982CEE765F/cache_data`, NOT the main checkout's `data/`** — the main checkout's `data/processed.db-wal` is filesystem-corrupted (2026-05-26 disk wedge; the canonical healthy cache moved to the scratch path — see `[[project-nse-data-quirks]]` § 3). The scratch cache's `raw/` symlinks back to the main checkout, so immutable provider downloads still carry over. The symlink commands MUST be `rm -rf data && ln -s /mnt/122CEE982CEE765F/cache_data data` and `ln -sf <main-checkout-abs>/.env .env` — not `ln -snf`, which creates a nested `data/data` because `data/.gitkeep` is tracked. Verify with `readlink data` returning the scratch path and `sqlite3 "$(readlink -f data)/processed.db" 'PRAGMA quick_check'` returning `ok`. Then run `git update-index --skip-worktree data/.gitkeep` once in the new worktree — otherwise the tracked `.gitkeep` shadowed by the symlink shows as a phantom modification and breaks `git rebase --autostash` / `git checkout`. The lockfile is shared via git, so `uv sync --frozen` in the worktree sets up an isolated `.venv/`.
...
- **Checking a background sub-agent's progress**: the harness reminder says not to `Read`/`cat`/`tail` the agent's JSONL transcript (`*.output`) — that guards against dumping the *whole* file (huge tool-result lines) into context, NOT against inspecting it. To report progress, **parse it with a bounded `python`/`jq`/`grep` query** that extracts only progress signals (event count, recent tool-use names, the last assistant text + last tool-result snippet, each truncated) and caps total output. Never print whole lines. See `.claude/memories/feedback-subagent-transcript-parsing.md` for the recipe.
...
- Don't treat the per-experiment verdict as automated — `report.md` is the agent's one-paragraph readout; the PASS/FAIL judgment is for the user. AUC ∈ [0.45, 0.55] **alone** is no longer sufficient to flag null — PR #28 + memo #138 (H=25 cross-market) found AUC=0.51 cells with weighted R-precision lift 1.46–2.12× (real top-tail signal masked by null AUC). Use the compound rule: AUC ∈ [0.45, 0.55] **AND** weighted R-precision lift < 1.2× → null signal; AUC ∈ [0.45, 0.55] **AND** weighted R-precision lift > 1.5× → **top-tail signal hidden by AUC** — investigate the prediction-extreme regime, don't dismiss. R-precision is per-day variable-K (K = R(d) = positives that day); panel-invariant, so it compares cleanly across markets/universes without the fixed-K bias P@k has. See `[[project-r-precision-methodology]]`.
