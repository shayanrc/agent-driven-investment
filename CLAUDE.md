...
- **`us_fundamentals` (data_pipelines v3)**: quarterly company fundamentals — `fetch("FUND:<TICKER>")` → one row per calendar quarter-end grid date (`date` = grid, `fiscal_period_end` = true period end, nullable `filed_date` only from EDGAR-served rows) with revenue / net_income / ocf / capex / fcf ($M), weighted shares (M), EPS. Chain macrotrends → SEC EDGAR → yfinance; universe derived from the us_equities YAMLs (no separate config); grid dates demanded only ≥45 d after quarter end (reporting lag). Values are as-first-published (per-cell first-written-wins merge). Derived ratios (PE/PS/P-FCF) deliberately NOT stored — a modeling-phase concern needing a price join + point-in-time lag on `filed_date`. Plan: `docs/data_pipelines/V3_US_FUNDAMENTALS_PLAN.md`; follow-ups: `V3_TBD.md`.
