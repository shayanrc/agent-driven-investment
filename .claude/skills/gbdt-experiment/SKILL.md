...
> **Anti-AUC + strong-top-1 cells → switch to the manual-tuning track (playbook rules 10-12).** If `sweep R-p@1 ≥ 0.5` AND `sweep test AUC ∈ [0.45, 0.55]` (the anti-AUC anomaly from CLAUDE.md "What not to do — gbdt"), the loop's val_brier objective is structurally misaligned: val_brier descends to a degenerate constant-predictor sink at the weighted base rate; the L1 tie-break (gap + Z) actively SELECTS the degenerate iter; val R-p@K is too noisy (Q_days × 1 pick/day is a tiny sample) to use as a substitute. **Don't drive the loop on val signals alone on these cells.** Pause iteration and execute the manual-tuning track instead: (1) cliff-cut FS, (2) single-knob response curves outside the loop, (3) mix-and-match around non-degenerate winners, (4) **eval-segment R-p@K validation** as the holdout oracle, (5) 1-shot test commit. The cell-5 manual track (`docs/gbdt/_211_*`) beat sweep R-p@1 by **+24%** (test 0.829 vs sweep 0.671) using a 6-tree depth-2 XGBoost (≤ 24 leaves, val_brier RIGHT at baseline) — a regime the loop's HP defaults never reach. See `[[project-gbdt-tuning-playbook]]` rules 10-12 for the full recipe + the per-stage scripts pattern.
