...
- **Atomic 7-state `wrapper.status` JSON**: `starting | running | restarting | heartbeat_stalled_killed | exited_ok | exited_failed | max_retries_hit`. These are the 7 distinct values written by `write_status` callsites in `scripts/gbdt/run_agent_loop_resumable.sh` (verifiable via `grep -n 'write_status' scripts/gbdt/run_agent_loop_resumable.sh`). Written via temp+rename so a partial read never returns junk; safe to poll from a Monitor loop.
