...
- [feedback-disk-wedge-pattern.md](feedback-disk-wedge-pattern.md) — NTFS `/mnt/122CEE982CEE765F` wedges when full → kernel D-state procs, unkillable, cascading; pre-flight disk check + non-NTFS cleanup paths
...
- [feedback-worktree-symlink-contract.md](feedback-worktree-symlink-contract.md) — Sub-agents can't `git worktree add` (sandbox blocks); parent pre-creates. Symlink: `rm -rf data && ln -s` (not `ln -snf` — creates nested `data/data` when data/ exists as dir)
