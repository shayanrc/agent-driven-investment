...
**How to apply:**
- For any new modeling change that produces forecasts, render the 8-anchor panel using `scripts/plot_forecast_from_date.py` and produce the coverage table per `FAT_TAIL_EVAL.md` §"v4 mandatory deliverable".
- Save figures under `docs/analog_mc/experiments/figs/<exp_id>_fat_tail/`.
- Anchor list is pinned at `results/analog_mc/data/fat_tail_eval_anchors.json`; regenerate via `scripts/select_fat_tail_anchors.py` only after a new canonical run shifts walk-forward fold boundaries.
