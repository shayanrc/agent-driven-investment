...
> **🔬 Ablation update (2026-05-18):** the 2×2 `(drift, conditional)` decomposition in [`ABLATION_STUDIES_REPORT.md`](ABLATION_STUDIES_REPORT.md) finds that **conditional sampling alone (Cell C) dominates aggregate CRPS** (−7.8% vs v1, −9.5% vs v2.1 default, −4.7% vs v2.2). Drift's only validated effect is PIT calibration; on aggregate CRPS it slightly hurts. The freeze of v2.1 as default may be revisited — see ABLATION_STUDIES_REPORT.md "Conclusions" for the three-way decision matrix.
