...
5. **Candidate set re-restriction** — at re-match for block `k+1`, the "effective forecast origin" advances by `block_length`. Should candidates be re-restricted to `d + block_length < origin_idx + k*block_length`, or kept as the original eligible set? **Original recommendation**: re-restrict. **Resolution during v2.2 implementation: KEEP THE v1 SET unchanged across blocks.** The "re-restrict" rule was less restrictive than the v1 `d + block_length < origin_idx` boundary for `k > 0`, which would admit candidates whose forward blocks contain *real post-origin returns* — a C6 walk-forward leak. The conservative correct rule keeps the v1 candidate set for every block; only the per-path distance computation changes. Documented at `src/analog_mc/sampling.py::generate_paths_conditional`.
