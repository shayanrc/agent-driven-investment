...
| R13 | Corporate-action treatment (splits + dividends, D24) | DD/target rules compare `close ≤ 0.95·anchor` and `close ≥ 1.10·anchor` — splits in raw `close` would fire fake DD exits (2:1 split ≈ −50% on the close). Mitigation: the runner anchors / MTMs on the `adj_close` column (NOT raw `close`) emitted by `data_pipelines`. The us_equities domain schema (`src/data_pipelines/domains/us_equities/schema.py:6-8, 44-46`) guarantees `adj_close` is at minimum split-adjusted (canonical `adjustment_quality ∈ {"full", "split_only"}`); pre-flight reads each universe ticker's per-provenance `adjustment_quality` from the `data_pipelines` cache `_meta.json` and fails loud if any in-window range carries an unrecognized value (defensive — should be unreachable under the US schema contract, but cheap to assert). Dividends excluded across all lines biases returns down by ~0.5-1% per year — internally consistent across strategy + benchmarks. Total-return (dividend-reinvested) basis is V1.1 (§10 Q8). |
