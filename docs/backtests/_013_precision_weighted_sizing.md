...
> **⚠️ CORRECTION (mechanism only — see [_023](_023_probweight_trim_fix.md)).** The *entry
> counts and mechanism* below ("prob_weight spreads across the whole universe, 486/890 entries →
> de-risker") are **bug-contaminated**: a ratchet-down-trim bug (`_notional_f` Kelly fall-through
> → trim every position to ~0 → daily churn) inflated prob_weight to a whole-universe
> closet-index. Fixed in `_023` — prob_weight holds ~K-scale books (sp500_50 K=20: **406 → 24
> entries**). **The VERDICT, however, stands:** the corrected rolling re-run confirms equal-K=3 ≥
> prob_weight on sp500/nifty **and r1k** (an apparent r1k reversal on a non-canonical
> predictions-only cell was retracted once re-run on the model-bearing champions). Read the
> *numbers/mechanism* here as superseded by `_023`; the *conclusion* (equal top-3 wins) holds.
