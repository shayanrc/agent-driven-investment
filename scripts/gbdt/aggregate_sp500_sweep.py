"""Aggregate the sp500 sweep (#193) into a single JSON data file.
...
Mirror of ``aggregate_nasdaq100_sweep.py`` (#192) and
``aggregate_russell1000_sweep.py`` (#188); only the universe prefix, output
filename, sweep/task labels, and branch label differ.

Output:
  ``results/gbdt/data/_193_sp500_sweep_results_data.json``
"""
...
OUTPUT_FILE = REPO_ROOT / "results/gbdt/data/_193_sp500_sweep_results_data.json"
...
def main() -> None:
    ...
    for cell_dir in sorted(EXPERIMENTS_DIR.iterdir()):
        if not cell_dir.is_dir() or not cell_dir.name.startswith("sp500_"):
            continue
        # Filter out smoketest (non-sweep companion).
        if cell_dir.name == "sp500_smoketest":
            continue
